...
const DocumentationVault: React.FC = () => {
  ...
  const handleBatchFileUpload = async () => {
    ...
    const tempItems: ActivityVaultItem[] = fileQueue.map(q => ({
      type: 'FILE',
      label: q.label,
      mimeType: q.file.type,
      nodeUrl: metadata?.storageNodeUrl // Assume current node
    }));
    
    const updatedGallery = [...items, ...tempItems];
    setItems(updatedGallery);

    // BACKGROUND SYNC
    const finalItems: ActivityVaultItem[] = [...items];
    ...
    setItems(finalItems);
    await handleSyncVault(finalItems);
    setFileQueue([]);
    setIsProcessing(false);
    showXeenapsToast('success', 'Files synchronized to vault');
  };
  ...
  const handleBatchLinkSave = async () => {
    ...
    setItems(updatedGallery); // Optimistic
    await handleSyncVault(updatedGallery);
    setLinkQueue([{ url: '', label: '' }]);
    showXeenapsToast('success', 'Links added to vault');
  };
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            {items.map((item, idx) => {
              ...
              const previewUrl = item.type === 'LINK' ? item.url : `https://lh3.googleusercontent.com/d/${item.fileId}`;
