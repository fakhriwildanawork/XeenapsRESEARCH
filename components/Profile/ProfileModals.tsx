...
/**
 * YEAR INPUT COMPONENT - REDESIGNED
 */
  ...
  return (
    <div className="flex items-stretch bg-gray-50 border border-gray-200 rounded-xl overflow-hidden focus-within:ring-2 focus-within:ring-[#004A74]/10 transition-all">
      <input 
        ...
        required={required}
        pattern="^[0-9]{4}$"
        className={`flex-1 px-5 py-3.5 bg-transparent border-none outline-none font-mono font-bold text-sm ${value === 'Present' ? 'text-emerald-600 italic' : 'text-[#004A74]'} placeholder:text-gray-300`}
      ...
      {allowPresent && (
        <button 
          ...
          className={`px-5 border-l border-gray-200 text-[9px] font-black uppercase tracking-widest transition-all ${
            value === 'Present' ? 'bg-emerald-500 text-white' : 'bg-white text-gray-400 hover:bg-gray-50'
          }`}
        >
          Present
        </button>
      ...
      <div className="bg-white rounded-[3rem] w-full max-w-xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
             ...
             <FormField label="Major / Study Program"><input className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-xl" value={formData.major} onChange={e => setFormData({...formData, major: e.target.value})} /></FormField>
          </div>
    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in">
      <div className="bg-white rounded-[3rem] w-full max-w-xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
             <FormField label="Job Position"><input required className="w-full px-5 py-4 bg-gray-50 border border-gray-200 rounded-xl" value={formData.position} onChange={e => setFormData({...formData, position: e.target.value})} /></FormField>
